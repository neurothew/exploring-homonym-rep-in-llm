...
def main(model_name, lang):
    fname_output = f"{dir_results}{os.sep}df_baseline_ambiguity-lexical_lang-{lang}_sense-same_model-{model_name.split('/')[-1]}"
    ...
    for target_word in list_target_words:
        ...
        list_sent_2 = df_homonymy.loc[df_homonymy.Word==target_word]['Sentence1'].values
        ...
        comb_sent_idx = list(combinations(range(n_sent), 2))
    ...
    df_sense1 = pd.concat(list_df)
    df_sense1["sense"] = 1

    list_df = []
    for target_word in list_target_words:
        list_sent_1 = df_homonymy.loc[df_homonymy.Word==target_word]['Sentence2'].values
    ...
    df_sense2 = pd.concat(list_df)
    df_sense2["sense"] = 2
    
    df = pd.concat((df_sense1, df_sense2))
    df["angle"] = np.arccos(df["cos_sim"]) * 180 / np.pi
    df_mean = df.groupby(by=[df.target_word, df.idx_sent_1, df.idx_sent_2, df.layer, df.sense]).agg({"cos_sim": "mean", "angle": "mean", "layer_rel": "first"}).assign(model=model_name.split('/')[-1]).reset_index()
